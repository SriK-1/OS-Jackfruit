...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(node, tmp, &monitor_list, list) {
        rss_bytes = get_rss_bytes(node->pid);
        ...
        if (rss_bytes > node->hard_limit_bytes) {
            kill_process(node->container_id, node->pid, node->hard_limit_bytes, rss_bytes);
            list_del(&node->list);
            kfree(node);
            continue;
        }

        if (rss_bytes > node->soft_limit_bytes && !node->soft_warned) {
            log_soft_limit_event(node->container_id, node->pid, node->soft_limit_bytes, rss_bytes);
